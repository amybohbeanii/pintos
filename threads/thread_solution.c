...
#include "threads/init.h" /*new*/
#include "threads/interrupt.h"
#include "threads/intr-stubs.h"
#include "threads/palloc.h"
#include "threads/switch.h"
#include "threads/synch.h"
#include "devices/timer.h" /*new*/
...
static fixed_point_t load_avg; /*new: load average. */
...
/*new function: adjust recent CPU of a thread based on load factor and recomputes its priority*/
static void adjust_recent_cpu(struct thread *t, void *aux){
  fixed_point_t load_factor = *(fixed_point_t *)aux;
  t->recent_cpu = fix_add(fix_mul(load_factor,t->recent_cpu),fix_int(t->nice));
...
if(thread_mlfqs){
  /*update load average. */
  if(time_ticks()%TIMER_FREQ==0){
    size_t ready_threads = list_size(&ready_list);
    if(t!=idle_thread)
      ready_threads++;
    load_avg = fix_add(fix_mul(fix_frac(59,60),load_avg),fix_mul(fix_frac(1,60),fix_int(ready_threads)));
  ]
...
/*  init_thread (t, name, priority); */
  init_thread(t,name,thread_mlfqs ? cur->priority: priority); /*new*/
  tid = t->tid = allocate_tid ();
  t->nice = cur->nice; /*new*/
  t->recent_cpu = cur->recent_cpu; /*new*/
  ...
  /* Add to run queue. */
  thread_unblock (t);
  if(priority>thread_get_priority()) /*new*/
    thread_yield(); /*new*/
  return tid;
}
...
/*new function that returns true if A has lower priority than B within a list of threads */
...
/*new function: Sets the current thread's priority to PRIORITY. */
void
...
/* New function: Sets the current thread's nice value to NICE. */
void
...
/* New function: Returns the current thread's nice value. */
int
...
/* New function: Returns 100 times the system load average. */
int
thread_get_load_avg (void) 
{
  int load_avg_int;
  enum intr_level level = intr_disable();
  load_avg_int = fix_roung(fix_scale(load_avg,100));
  intr_set_level(level);
  return load_avg_int;
}

/* New function; Returns 100 times the current thread's recent_cpu value. */
int
...
/*  t->priority = priority; *
  t->priority = t->normal_priority = priority; /*new*/
  t->magic = THREAD_MAGIC;
  sema_init(&t->timer_sema,0); /*new*/
  list_init(&t->donors); /*new*/
